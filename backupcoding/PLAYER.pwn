...
CMD:rent(playerid)
{
	...
	if(IsPlayerInRangeOfPoint(playerid,2.0,1682.7796,-2264.0961,13.5116) || IsPlayerInRangeOfPoint(playerid,2.0,1811.7331,-1876.9458,13.5781))
	{
		ShowPlayerDialog(playerid,DIALOG_RENT_BIKE,DIALOG_STYLE_TABLIST,"Bike Rental","1. BMX\t\t{00ff00}$2\n{ffffff}2. Mountain Bike\t\t{00ff00}$5","Rent","Exit");
	}
	else
	{
...
CMD:credits(playerid)
{
	...
	strcat(line3, ""LB_E"Gamemode Developer: "YELLOW_E"Dandy_Prasetyo\n");
...
CMD:givedrivelic(playerid, params[])
{
	if(pData[playerid][pFaction] != 1) return Error(playerud,"Kamu bukan anggota polisi");
	new id;
	if(sscanf(params,"i",id)) return Usage(playerid,"/givedrivelic [playerid]")
	//if(!IsPlayerInRangeOfPoint(playerid, 3.0, 252.22, 117.43, 1003.21)) return Error(playerid, "Anda harus berada di SAPD!");
	if(pData[id][pDriveLic] != 0) return Error(playerid, "Target sudah memiliki Driving License!");
	if(GetPlayerMoney(id) < 200) return Error(playerid, "Target butuh $200 untuk membuat Driving License.");
	new sext[40], mstr[128];
	if(pData[id][pGender] == 1) { sext = "Laki-Laki"; } else { sext = "Perempuan"; }
	format(mstr, sizeof(mstr), "{FFFFFF}Nama: %s\nNegara: San Andreas\nTgl Lahir: %s\nJenis Kelamin: %s\nBerlaku hingga 14 hari!", pData[playerid][pName], pData[playerid][pAge], sext);
	ShowPlayerDialog(id, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Driving License", mstr, "Tutup", "");
	pData[id][pDriveLic] = 1;
	pData[id][pDriveLicTime] = gettime() + (15 * 86400);
	GivePlayerMoneyEx(id, -200);
	Server_AddMoney(200);
	return 1;
}
