...
*/
	...
	if(dialogid == DIALOG_HBEMODE)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					/*(playerid, pData[playerid][sphungrybar]);
					...
					{
						PlayerTextDrawHide(playerid, PlayerTD[playerid][txd]);
					}
					for(new txd; txd < 7; txd++)
					{
						PlayerTextDrawHide(playerid, newtdhbe[playerid][txd]);
					}
				...
				case 1:
				{
					...
					for(new txd; txd < 7; txd++)
					{
						PlayerTextDrawShow(playerid, newtdhbe[playerid][txd]);
					}
				}
				case 2:
				{
	...
	if(dialogid == DIALOG_MY_BISNIS)
	{
		...
		ShowPlayerDialog(playerid, BISNIS_INFO, DIALOG_STYLE_LIST, "{FF0000}SAMPINDORP {0000FF}Bisnis", "Show Information\nTrack Bisnis", "Select", "Cancel");
		return 1;
	}
	...
	if(dialogid == DIALOG_MY_HOUSES)
	{
		...
		ShowPlayerDialog(playerid, HOUSE_INFO, DIALOG_STYLE_LIST, "{FF0000}SAMPINDORP {0000FF}Houses", "Show Information\nTrack House", "Select", "Cancel");
		return 1;
	}
			...
			switch(listitem)
			{
				case 0: //slot 1
				{
					...
					else
					{
						...
						ShowPlayerDialog(playerid, DIALOG_TOYEDIT, DIALOG_STYLE_LIST, ""RED_E"SAMPINDORP "WHITE_E"Player Toys", string, "Select", "Cancel");
					}
				}
				case 1: //slot 2
				{
					...
					else
					{
						...
						pToys[playerid][pData[playerid][toySelected]][toy_rx], pToys[playerid][pData[playerid][toySelected]][toy_ry], pToys[playerid][pData[playerid][toySelected]][toy_rz]);
						ShowPlayerDialog(playerid, DIALOG_TOYEDIT, DIALOG_STYLE_LIST, ""RED_E"SAMPINDORP "WHITE_E"Player Toys", string, "Select", "Cancel");
						//ShowPlayerDialog(playerid, DIALOG_TOYEDIT, DIALOG_STYLE_LIST, ""RED_E"SAMPINDORP "WHITE_E"Player Toys", ""dot"Edit Toy Position\n"dot"Change Bone\n"dot""GREY_E"Remove Toy\n"dot"Share Toy Pos", "Select", "Cancel");
					}
				}
				case 2: //slot 3
				{
		...
		if(response)
		{
			switch (listitem) 
			{
				...
				case 3:
				{
					...
					ShowPlayerDialog(playerid, DIALOG_TOYVIP, DIALOG_STYLE_LIST, ""RED_E"SAMPINDORP "WHITE_E"VIP Toys", string, "Select", "Cancel");
				}
			}
		}
	}
	...
	if(dialogid == DIALOG_ATM)
	{
		...
		switch(listitem)
		{
			...
			{
				new mstr[512];
				format(mstr, sizeof(mstr), "{F6F6F6}You have "LB_E"%s {F6F6F6}in your bank account.", FormatMoney(debData[SaveList][Dsaldo]));
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""LB_E"Bank", mstr, "Close", "");
			}
			case 1: // Withdraw
			{
				new mstr[128];
				format(mstr, sizeof(mstr), ""WHITE_E"My Balance: "LB_E"%s", FormatMoney(debData[SaveList][Dsaldo]));
	...
	if(dialogid == DIALOG_ATMWITHDRAW)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					if(debData[SaveList][Dsaldo] < 50)
						return Error(playerid, "Not enough balance!");
					
					GivePlayerMoneyEx(playerid, 50);
					debData[SaveList][Dsaldo] -= 50;
					Info(playerid, "ATM withdraw "LG_E"$50");
				}
				case 1:
				{
					if(debData[SaveList][Dsaldo] < 200)
						return Error(playerid, "Not enough balance!");
					
					GivePlayerMoneyEx(playerid, 200);
					debData[SaveList][Dsaldo] -= 200;
					Info(playerid, "ATM withdraw "LG_E"$200");
				}
				case 2:
				{
					if(debData[SaveList][Dsaldo] < 500)
						return Error(playerid, "Not enough balance!");
					
					GivePlayerMoneyEx(playerid, 500);
					debData[SaveList][Dsaldo] -= 500;
					Info(playerid, "ATM withdraw "LG_E"$500");
				}
				case 3:
				{
					if(debData[SaveList][Dsaldo] < 1000)
						return Error(playerid, "Not enough balance!");
					
					GivePlayerMoneyEx(playerid, 1000);
					debData[SaveList][Dsaldo] -= 1000;
					Info(playerid, "ATM withdraw "LG_E"$1.000");
				}
				case 4:
				{
					if(debData[SaveList][Dsaldo] < 5000)
						return Error(playerid, "Not enough balance!");
					
					GivePlayerMoneyEx(playerid, 5000);
					debData[SaveList][Dsaldo] -= 5000;
	...
	if(dialogid == DIALOG_BANKDEPOSIT)
	{
		...
		foreach(new id: kartu)
		{
			if(id != -1)
			{
				if(debData[id][Drek] == pData[playerid][pBankRek])
				{
					debData[id][Dsaldo] += amount;
					GivePlayerMoneyEx(playerid, -amount);
					Info(playerid, "You have successfull deposit %s", FormatMoney(amount));
				}
			}
		}
		...
		foreach(new id: kartu)
		{
			if(id != -1)
			{
				if(debData[id][Drek] == pData[playerid][pBankRek])
				{
					debData[id][Dsaldo] -= amount;
					GivePlayerMoneyEx(playerid, amount);
					Info(playerid, "You have successfull withdraw %s", FormatMoney(amount));
				}
			}
		}
		/*else
	...
	{
		if(!response) return true;
		if(response) return Error(playerid, "Dalam perbaikan");
		/*new amount = floatround(strval(inputtext));
		...
		if(response)
		{
			...
			if(rows) 
			{
				...
				foreach(new ii: kartu)
				{
					if(ii != -1)
					{
						if(debData[ii][Drek] == pData[playerid][pBankRek])
						{
							debData[ii][Dsaldo] += hasil;
						}
					}
				}
	...
	if(dialogid == DIALOG_RENT_BIKE)
	{
		...
		switch(listitem)
		{
			case 0:
			{
				if(IsPlayerInRangeOfPoint(playerid,2.0,1682.98,-2309.55,13.5406))
				{
					Rented_Bike[playerid] = CreateVehicle(481,1684.1969,-2319.4939,13.3828,92.4917,3,4,120000,0);
				}
				if(IsPlayerInRangeOfPoint(playerid,2.0,1811.7331,-1876.9458,13.5781))
				{
					Rented_Bike[playerid] = CreateVehicle(481,1817.5351,-1877.0605,13.4140,185.6917,3,4,120000,0);
				}
			...
			case 1:
			{
				if(IsPlayerInRangeOfPoint(playerid,2.0,1682.98,-2309.55,13.5406))
				{
					Rented_Bike[playerid] = CreateVehicle(510,1684.1969,-2319.4939,13.3828,92.4917,3,4,120000,0);
				}
				if(IsPlayerInRangeOfPoint(playerid,2.0,1811.7331,-1876.9458,13.5781))
				{
					Rented_Bike[playerid] = CreateVehicle(510,1817.5351,-1877.0605,13.4140,185.6917,3,4,120000,0);
				}
				GivePlayerMoneyEx(playerid, - 5);
			}
		}
		pData[playerid][pRented] = 1;
		PutPlayerInVehicle(playerid, Rented_Bike[playerid], 0);
	}
	if(dialogid == DIALOG_EDITFRAKSI)
	{
		if(!response) return 1;
		list_item = listitem+1;
		ShowPlayerDialog(playerid, DIALOG_FRAKSI, DIALOG_STYLE_LIST, "Edit Fraksi", "1. Nama Fraksi\n2. Rank Fraksi", "OK", "Exit");
	}
	if(dialogid == DIALOG_FRAKSI)
	{
		if(!response) return 1;
		new nama_fraksi[1288];
		format(nama_fraksi, sizeof(nama_fraksi), "Nama fraksi: %s", facData[list_item][FacName]);
		switch(listitem)
		{
			case 0: ShowPlayerDialog(playerid, DIALOG_NAMAFRAKSI, DIALOG_STYLE_INPUT, nama_fraksi, "Masukkan nama untuk fraksi ini:", "Go", "exit");
			case 1:
			{
				String = "";
				new rstr[128];
				for(new i = 1; i < 8; i++)
				{
					format(rstr, sizeof(rstr), "%i. %s\n", i, facRank[list_item][i]);
					strcat(String, rstr);
				}
				ShowPlayerDialog(playerid, DIALOG_NAMARANK, DIALOG_STYLE_LIST, nama_fraksi, String, "OK", "exit");
			}
		}
	}
	if(dialogid == DIALOG_NAMAFRAKSI)
	{
		if(!response) return 1;
		format(facData[list_item][FacName],128,inputtext);
		SendClientMessageEx(playerid, -1, "{800080}[SUKSES] {ffffff}Nama fraksi berhasil di edit %s", facData[list_item][FacName]);
		Faction_Save(list_item);
	}
	if(dialogid == DIALOG_NAMARANK)
	{
		if(!response) return 1;

		SetPVarInt(playerid, "rank_id", listitem+1);

		ShowPlayerDialog(playerid, DIALOG_EDITRANK, DIALOG_STYLE_INPUT, "Nama baru rank", "Masukkan nama baru rank:", "Lanjut", "Keluar");
	}
	if(dialogid == DIALOG_EDITRANK)
	{
		if(!response) return 1;

		new rankid = GetPVarInt(playerid, "rank_id");

		format(facRank[list_item][rankid], 50, inputtext);

		SendClientMessageEx(playerid, -1, "{800080}[SUCCESS] {ffffff} Nama rank diubah ke %s", inputtext);

		callcmd::fpanel(playerid);
	...
	if(dialogid == DIALOG_PAYBILL)
	{
		...
		foreach(new fif: fraksi)
		{
			if(fif != 0)
			{
				if(fif == bilData[idtag][bilType])
				{
					facData[fif][FacSafe] += bilData[idtag][bilammount];
					Faction_Save(fif);
	...
	if(dialogid == DIALOG_FACSAFE)
	{
		if(!response) return 1;
		switch(listitem)
		{
			case 0: ShowPlayerDialog(playerid, DIALOG_FACTARIK, DIALOG_STYLE_INPUT, "faction safe withdraw", "Masukkan jumlah uang yang ingin kamu ambil:", "Next", "exit");
			case 1: ShowPlayerDialog(playerid, DIALOG_FACMASUK, DIALOG_STYLE_INPUT, "faction safe deposit", "Masukkan jumlah uang yang ingin kamu masukkan:", "Next", "exit");
		}
	}
	if(dialogid == DIALOG_FACTARIK)
	{
		if(!response) return 1;
		foreach(new ii: fraksi)
		{
			if(ii != 0)
			{
				if(ii == pData[playerid][pFaction])
				{
					if(strval(inputtext) > facData[ii][FacSafe]) return Error(playerid, "Uang di brankas tidak cukup");
					facData[ii][FacSafe] -= strval(inputtext);
					GivePlayerMoneyEx(playerid, strval(inputtext));
					Faction_Save(ii);
					Info(playerid, "Kamu mengambil uang {00ff00}%s {ffffff}dari brankas %s", FormatMoney(strval(inputtext)), facData[ii][FacName]);
				}
			}
		}
	}
	if(dialogid == DIALOG_FACMASUK)
	{
		if(!response) return 1;
		foreach(new ii: fraksi)
		{
			if(ii != 0)
			{
				if(ii == pData[playerid][pFaction])
				{
					if(GetPlayerMoney(playerid) > strval(inputtext)) Error(playerid, "Tidak cukup");
					GivePlayerMoneyEx(playerid, -strval(inputtext));
					facData[ii][FacSafe] += strval(inputtext);
					Faction_Save(ii);
					Info(playerid, "Kamu menaruh uang {00ff00}%s {ffffff}ke brankas %s", FormatMoney(strval(inputtext)), facData[ii][FacName]);
				}
			}
		}
	}
	if(dialogid == DIALOG_SETLEADER)
	{
		...
		new otherid = GetPVarInt(playerid, "targetid");
		foreach(new ii: fraksi)
		{
			if(ii != -1)
			{
		...
		{
			pData[otherid][pFaction] = facid;
			pData[otherid][pFactionLead] = facid;
			pData[otherid][pFactionRank] = 7;
			Servers(playerid, "You have set %s's faction %s with leader.", pData[otherid][pName], facData[facid][FacName]);
			Servers(otherid, "%s has set your faction to %s with leader.", pData[playerid][pName], facData[facid][FacName]);
		}
		DeletePVar(playerid, "targetid");
		
	}
	if(dialogid == DIALOG_BM)
	{
		if(!response) return 1;
		switch(listitem)
		{
			...
			case 3:
			{
				if(pData[playerid][pMaterial] <= 5 && pData[playerid][pComponent] <= 10 && pData[playerid][pGas] <= 1) return Error(playerid, "You need: Material 5, Component 10, Gas 1");

				pData[playerid][pCraftTime] = 120;
				
				pData[playerid][pCraft] = SetTimerEx("Craftdrill", 1000, true, "d", playerid);
			}
		}
	}
	...
	if(dialogid == DIALOG_CREATEATM)
	{
		if(!response) return 1;

		if(!IsNumeric(inputtext)) return Error(playerid, "Masukkan pin hanya angka");
		if(strlen(inputtext) < 4 || strlen(inputtext) > 4) return Error(playerid, "Input 4 digit pin");
		new dquery[256];
		new idd = Iter_Free(kartu);
		if(idd == -1) return SendClientMessageEx(playerid, -1, ""RED_E"[ERROR] {ffffff} Kamu tidak bisa membuat kartu atm");
		debData[idd][Dholder] = pData[playerid][pID];
		format(debData[idd][Dname], 128, "%s", pData[playerid][pName]);
		format(debData[idd][Dnumber], 128, "%s", SaveInter);
		debData[idd][Dpin] = strval(inputtext);
		debData[idd][Dsaldo] = 200;
		Iter_Add(kartu, idd);
		pData[playerid][pDebit] += 1;
		debData[idd][Drek] = pData[playerid][pBankRek];

		mysql_format(g_SQL, dquery, sizeof(dquery), "INSERT INTO `debit` (`did`,`dpin`) VALUES ('%d','%d')", idd, debData[idd][Dpin]);
		mysql_tquery(g_SQL, dquery, "OnDebitCreate", "d", idd);
	}
	if(dialogid == DIALOG_AKSESATM)
	{
		if(!response) return 1;

		new step = 0, idkartu;
		foreach(new idk: kartu)
		{
			if(idk != -1)
			{
				if(debData[idk][Dholder] == pData[playerid][pID])
				{
					if(step == listitem)
					{
						idkartu = idk;
		...
		SaveList = idkartu;
		ShowPlayerDialog(playerid, DIALOG_AKSESATM_PIN, DIALOG_STYLE_INPUT, "Login", "Silahkan masukkan pin kartu atm kamu:", "Next", "Exit");
		/*format(tstr, sizeof(tstr), ""ORANGE_E"Card No.: %s\tNo Rek: "LB_E"%d", debData[idkartu][Dnumber], pData[playerid][pBankRek]);
		ShowPlayerDialog(playerid, DIALOG_ATM, DIALOG_STYLE_LIST, tstr, "Check Balance\nWithdraw Money\nTransfer Money\nSign Paycheck\tDeposit to Card", "Select", "Cancel");*/
	}
	if(dialogid == DIALOG_AKSESATM_PIN)
	{
		if(!response) return 1;

		if(strval(inputtext) != debData[SaveList][Dpin]) return Error(playerid, "Invalid Pin!");
		new tstr[128];
		format(tstr, sizeof(tstr), ""ORANGE_E"Card No.: %s\tNo Rek: "LB_E"%d", debData[SaveList][Dnumber], pData[playerid][pBankRek]);
		ShowPlayerDialog(playerid, DIALOG_ATM, DIALOG_STYLE_LIST, tstr, "Check Balance\nWithdraw Money\nTransfer Money\nSign Paycheck\tDeposit to Card", "Select", "Cancel");
	}
	if(dialogid == DIALOG_HP)
	{
		if(!response) return 1;

		switch(listitem)
		{
			case 0: ShowPlayerDialog(playerid, DIALOG_HP1, DIALOG_STYLE_INPUT, "Masukkan Nomor HP", "Masukkan nomor HP tujuan:", "Next", "Exit");
			case 1: ShowPlayerDialog(playerid, DIALOG_HP2, DIALOG_STYLE_INPUT, "Masukkan Nomor HP", "Masukkan nomor HP tujuan:", "Next", "Exit");
			case 2:
			{
				new hpinfo[128];
				format(hpinfo, sizeof(hpinfo), ""WHITE_E"Phone Owner: "YELLOW_E"%s\n\n"WHITE_E"Phone Number: "YELLOW_E"%d\n\n"WHITE_E"Phone Credit: "YELLOW_E"%d", pData[playerid][pName], pData[playerid][pPhone], pData[playerid][pPhoneCredit]);
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Phone Info", hpinfo, "OK", "");
			}
		}
	}
	if(dialogid == DIALOG_HP1)
	{
		if(!response) return 1;

		pData[playerid][pSavedNum] = strval(inputtext);

		ShowPlayerDialog(playerid, DIALOG_HP3, DIALOG_STYLE_INPUT, "Masukkan Pesan", "Tulis Pesan Yang Ingin disampaikan:", "Next", "Exit");
	}
	if(dialogid == DIALOG_HP3)
	{
		if(!response) return 1;

		foreach(new ii: Player)
		{
			if(pData[ii][pPhone] == pData[playerid][pSavedNum])
			{
				if(ii == INVALID_PLAYER_ID || !IsPlayerConnected(playerid)) return Error(playerid, "This Number is not active");
				SendClientMessageEx(playerid, COLOR_YELLOW, "[SMS to %d]"WHITE_E" %s", pData[playerid][pSavedNum], inputtext);
	...
	if(dialogid == DIALOG_HP2)
	{
		if(!response) return 1;

		pData[playerid][pSavedNum] = strval(inputtext);

		if(pData[playerid][pSavedNum] == 933)
		{
