...
CMD:getcords(playerid)
{
	if(!pData[playerid][pAdmin]) return Error(playerid,"You're not admin");
	new Float:ax, Float:ay, Float:az, Float:aa;
	GetPlayerPos(playerid, ax, ay, az);
	GetPlayerFacingAngle(playerid, aa);
	SendClientMessageEx(playerid,0xFFFFFFFF,"{0012dd}[POS]: {ffffff}%.4f,%.4f,%.4f\n{01frdd}[Angle]: {ffffff}%f\n[VW]: %d\t[INT]: %d",ax,ay,az,aa,GetPlayerVirtualWorld(playerid),GetPlayerInterior(playerid));
	printf("%s: [POS] %.4f,%.4f,%.4f | [Angle] %f | [VW] %d | [INT] %d",pData[playerid][pName],ax,ay,az,aa,GetPlayerVirtualWorld(playerid),GetPlayerInterior(playerid));
	return 1;
}
...
CMD:revive(playerid, params[])
{
	...
	PlayerTextDrawHide(otherid, tdblur_0[otherid]);
...
CMD:setleader(playerid, params[])
{
    ...
    if(sscanf(params, "ud", otherid))
        return Usage(playerid, "/setleader [playerid/PartOfName]");
	...
	SetPVarInt(playerid, "targetid", otherid);

	new pemimpin[589], count = 0;
	foreach(new ii: fraksi)
	{
		if(ii != -1)
		{
			format(pemimpin, sizeof(pemimpin), "%s%d. %s\n", pemimpin, count, facData[ii][FacName]);
			count++;
		}
	}

	ShowPlayerDialog(playerid, DIALOG_SETLEADER, DIALOG_STYLE_LIST, "Set Leader", pemimpin, "Next", "Exit");

    /*if(fid < -1 || fid >= MAX_FACTIONS)
        return Error(playerid, "You have specified an invalid faction ID 0 - %d.", MAX_FACTIONS);
...
CMD:tpm(playerid)
{
	...
	if(pData[playerid][pClickMap] == 1)
	{
		SetPlayerPos(playerid, GetPVarFloat(playerid, "pointx"), GetPVarFloat(playerid, "pointy"), GetPVarFloat(playerid, "pointz"));
		SetPlayerInterior(playerid, 0);
		SetPlayerVirtualWorld(playerid, 0);

		pData[playerid][pClickMap] = 0;
		DeletePVar(playerid, "pointx");
		DeletePVar(playerid, "pointy");
		DeletePVar(playerid, "pointz");
	}
	else
	{
		Error(playerid, "Tanadai di map terlebih dahulu");
	}
	return 1;
}