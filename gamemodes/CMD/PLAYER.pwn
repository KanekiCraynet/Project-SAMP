...
CMD:phone(playerid)
{
	if(!pData[playerid][pPhone]) return Error(playerid, "You dont have a phone");
	
	ShowPlayerDialog(playerid, DIALOG_HP, DIALOG_STYLE_LIST, "Phone Menu", "1. SMS\n2. Call\n3. Phone Information", "Next", "Exit");

	return 1;
}
CMD:rent(playerid)
{
	if(pData[playerid][pRented] == 1) return Error(playerid,"You're alreadyrent a bike");
	if(IsPlayerInRangeOfPoint(playerid,2.0,1682.98,-2309.55,13.5406) || IsPlayerInRangeOfPoint(playerid,2.0,1811.7331,-1876.9458,13.5781))
	{
		ShowPlayerDialog(playerid,DIALOG_RENT_BIKE,DIALOG_STYLE_TABLIST_HEADERS,"Bike Rental","Nama\t\tHarga\n1. BMX\t\t{00ff00}$2\n{ffffff}2. Mountain Bike\t\t{00ff00}$5","Rent","Exit");
	}
	else
	{
		Error(playerid,"You're not near any rental pos");
	}
	return 1;
}
CMD:unrent(playerid)
{
	if(pData[playerid][pRented] == 1)
	{
		DestroyVehicle(Rented_Bike[playerid]);
		pData[playerid][pRented] = 0;
	}
	else
	{
		Error(playerid,"You didn't rent any bike");
	}
	return 1;
}
...
{
	new id;
	if(pData[playerid][pFaction] != 2) return Error(playerid,"You're not Government staff");
	if(sscanf(params,"d",id)) return Usage(playerid,"/giveidcard <Player ID>");
	//if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1392.77, -22.25, 1000.97)) return Error(playerid, "Anda harus berada di City Hall!");
	if(pData[id][pIDCard] != 0) return Error(playerid, "Target sudah memiliki ID Card!");
	if(GetPlayerMoney(id) < 50) return Error(playerid, "Target butuh $50 untuk membuat ID Card");
	new sext[40], mstr[128];
	if(pData[id][pGender] == 1) { sext = "Laki-Laki"; } else { sext = "Perempuan"; }
	format(mstr, sizeof(mstr), "{FFFFFF}Nama: %s\nNegara: San Andreas\nTgl Lahir: %s\nJenis Kelamin: %s\nBerlaku hingga 14 hari!", pData[playerid][pName], pData[playerid][pAge], sext);
	ShowPlayerDialog(id, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "ID-Card", mstr, "Tutup", "");
	pData[id][pIDCard] = 1;
	pData[id][pIDCardTime] = gettime() + (15 * 86400);
	GivePlayerMoneyEx(id, -50);
	Server_AddMoney(25);
	return 1;
}
...
{
	if(Robdelay > 0) return Error(playerid, "Bank telah dirampok harap tunggu sampai: %s", ReturnTimelapse(gettime(), Robdelay));
...
CMD:accept(playerid, params[])
{
	if(IsPlayerConnected(playerid)) 
	{
		...
		else if(strcmp(params,"drag",true) == 0)
		{
			...
			pData[playerid][pDragTimer] = SetTimerEx("DragUpdate", 120, true, "ii", dragby, playerid);
...
{
	if(pData[playerid][pDrill] <= 0) return Error(playerid, "You need atleast 1 driller");
	new onduty;
	foreach(new ii: Player)
	{
		if(pData[ii][pFaction] == 1 && pData[ii][pOnDuty] == 1) onduty++;
	}
	if(onduty == 0) return Error(playerid, "No Police on duty");
	if(Robdelay == 0)
	{
		if(!IsPlayerInRangeOfPoint(playerid, 2.5, 2255.92, -1747.33, 1014.77)) return Error(playerid, "Kamu harus berada dibank");
		if(pData[playerid][pLevel] < 3) return Error(playerid, "Dibutuhkan level 3 ke atas untuk merampok");
		pData[playerid][pRobTime] = SetTimerEx("WaktuPerampokan", 1000, true, "d", playerid);
		RobTimer[playerid] = RandomEx(50, 433);
	}
	else
	{
		Error(playerid, "This bank has been stolesn please wait until: %s", ReturnTimelapse(gettime(), Robdelay));
	}
	return 1;
}
CMD:blackmarket(playerid, params[])
{
	if(IsPlayerInRangeOfPoint(playerid, 2.1, -1116.5280, -1636.9052, 76.3671))
	{
		ShowPlayerDialog(playerid, DIALOG_BM, DIALOG_STYLE_TABLIST_HEADERS, "Black Market","Name\tPrice\n\
		AK-47\t{00ff00}$450{ffffff}/250 Ammo\n\
		M4\t{00ff00}$750{ffffff}/250 Ammo\n\
		Desert Eagle\t{00ff00}$350{ffffff}/250 Ammo\n\
		Driller\tCraft","Next","Exit");
	}
	else
	{
		Error(playerid, "You not near black market");
	}
	return 1;
}
...
CMD:cekid(playerid)
{
	...
	if(vehid == INVALID_VEHICLE_ID || !IsValidVehicle(vehid)) return Error(playerid, "Not found");

	SendClientMessageEx(playerid, -1, "This Vehiv=cle id: %d", vehid);

	return 1;
}
CMD:ujicoba(playerid)
{
	
	CreatePlayerLoadingBar(playerid, 50, "Testing...");
