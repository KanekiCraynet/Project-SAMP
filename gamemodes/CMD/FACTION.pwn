...
CMD:factionhelp(playerid)
{
	if(pData[playerid][pFaction] == 1)
	{
		...
		strcat(str, ""BLUE_E"SAPD: /takemarijuana /spike /destroyspike /destroyallspike\n");
...
CMD:r(playerid, params[])
{
	...
	foreach(new ii: fraksi)
	{
		if(ii != 0)
		{
			...
			{
				SendFactionMessage(ii, COLOR_RADIO, "** [%s Radio] %s(%d) %s: %s", facData[ii][FacName], facRank[ii][pData[playerid][pFactionRank]], pData[playerid][pFactionRank], pData[playerid][pName], text);
				format(mstr, sizeof(mstr), "[<RADIO>]\n* %s *", text);
				SetPlayerChatBubble(playerid, mstr, COLOR_RADIO, 10.0, 3000);
			}
		}
	...
	for(new fid = 1; fid < MAX_FACTIONS; fid++)
	{
		foreach(new ii: fraksi)
		{
			if(ii != 0)
			{
				if(ii == pData[playerid][pFaction])
				{
					SendFactionMessage(fid, 0xFFD7004A, "** [%s] %s(%d) %s: %s", facData[ii][FacName], facRank[ii][pData[playerid][pFactionRank]], pData[playerid][pFactionRank], pData[playerid][pName], text);
					format(mstr, sizeof(mstr), "[<DEPARTEMENT>]\n* %s *", text);
					SetPlayerChatBubble(playerid, mstr, 0xFFD7004A, 10.0, 3000);
				}
			}
...
CMD:gov(playerid, params[])
{
	...
	foreach(new fi: fraksi)
	{
		if(fi != 0)
		{
			...
			{
				new lstr[1024];
				format(lstr, sizeof(lstr), "** %s: %s(%d) %s: %s **", facData[fi][FacName], facRank[fi][pData[playerid][pFactionRank]], pData[playerid][pFactionRank], pData[playerid][pName], params);
...
CMD:setrank(playerid, params[])
{
	...
	if(sscanf(params, "ud", otherid, rank))
        return Usage(playerid, "/setrank [playerid/PartOfName] [Rank(1-7)");
	...
	if(rank < 1 || rank > 7)
		return Error(playerid, "rank must 1 - 7 only");
...
CMD:sapdonline(playerid, params[])
{
	...
	foreach(new i: Player)
	{
		if(pData[i][pFaction] == 1)
		{
			...
			format(lstr, sizeof(lstr), "%s%s(%d)\t%s(%d)\t%s\t%d detik", lstr, pData[i][pName], i, facRank[pData[i][pFaction]][pData[i][pFactionRank]], pData[i][pFactionRank], duty, pData[i][pOnDutyTime]);
