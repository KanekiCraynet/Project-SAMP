...
CMD:atm(playerid, params[])
{
	...
	if(id > -1)
	{
		...
		new datm[256], count = 1, bool:status = false;
		format(datm, sizeof(datm), "No.\tCard Number\tSaldo\n");
		foreach(new i:kartu)
		{
			if(i != -1)
			{
				if(debData[i][Dholder] == pData[playerid][pID])
				{
					format(datm, sizeof(datm), "%s%d.\t%s\t{00ff00}%s", datm, count, debData[i][Dnumber], FormatMoney(debData[i][Dsaldo]));
					count++;
					status = true;
				}
			}
		...
		if(status)
		{
			ShowPlayerDialog(playerid, DIALOG_AKSESATM, DIALOG_STYLE_TABLIST_HEADERS, "Pilih Kartu", datm, "Pilih", "Keluar");
		}
		else
		{
