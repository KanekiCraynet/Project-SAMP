...
function OnPlayerDataLoaded(playerid, race_check)
{
	...
	if(cache_num_rows() > 0)
	{
		...
		format(string, sizeof string, "{800080}Welcome to Dewata Roleplay!\n{ffffff}Your Account: %s\nStatus: {00ff00}Registered\n{ffffff}You can input your passowrd to login:", pData[playerid][pName]);
	...
	{
		format(string, sizeof string, "{800080}Welcome to Dewata Roleplay!\n{ffffff}Your Account: %s\nStatus: {ffff00}Not Registered\n{ffffff}You can register by input your passowrd below:", pData[playerid][pName]);
...
{
	new aname[MAX_PLAYER_NAME], email[40], age[128], regdate[50], lastlogin[50];
	...
	cache_get_value_name_int(0, "drill", pData[playerid][pDrill]);
...
{	
	...
	foreach(new ii : Player)
	{
		...
		if(Robdelay != 0 && Robdelay <= gettime())
		{
			SendClientMessageToAll(-1, "{800080}INFO: {ffffff} Bank telah dibuka!");
			Robdelay = 0;
			Server_Save();
		}
...
function WaktuPerampokan(playerid)
{
	if(IsPlayerInRangeOfPoint(playerid, 2.5, 2255.92, -1747.33, 1014.77))
	{
		if(RobTimer[playerid] > 0)
		{
			...
			RobTimer[playerid]--;
			PlayerTextDrawShow(playerid, tdrob[playerid]);
			new ms[128];
			format(ms, sizeof(ms), "Bank Robbery: ~r~%d ~w~Seconds", RobTimer[playerid]);
			PlayerTextDrawSetString(playerid, tdrob[playerid], ms);
			if(playerid == INVALID_PLAYER_ID || !IsPlayerConnected(playerid))
			{
				KillTimer(pData[playerid][pRobTime]);
			}
		}
		else
		{
			pData[playerid][pDrill] -= 1;
			KillTimer(pData[playerid][pRobTime]);
			PlayerTextDrawHide(playerid, tdrob[playerid]);
			RobTimer[playerid] = RandomEx(50, 433);
			// Robdelay = gettime() + 3123;
			new rand = RandomEx(1, 4);
			switch(rand)
			{
				case 1:
				{
					new Rand = RandomEx(100, 250);
					new randdelay = RandomEx(90, 334);
					pData[playerid][pRedMoney] += Rand;
					Robdelay = gettime() + randdelay;
					Server_MinMoney(Rand);
					Server_Save();
					CreateServerPoint();
					new mes[128];
					format(mes, sizeof(mes), "You stolen ~r~%s", FormatMoney(Rand));
					InfoTD_MSG(playerid, 3500, mes);
				}
				case 2:
				{
					new Rand = RandomEx(300, 550);
					new randdelay = RandomEx(133, 455);
					pData[playerid][pRedMoney] += Rand;
					Robdelay = gettime() + randdelay;
					Server_MinMoney(Rand);
					Server_Save();
				...
				case 3:
				{
					new Rand = RandomEx(667, 896);
					new randdelay = RandomEx(588, 856);
					pData[playerid][pRedMoney] += Rand;
					Robdelay = gettime() + randdelay;
					Server_MinMoney(Rand);
					Server_Save();
				...
				case 4:
				{
					new Rand = RandomEx(995, 1334);
					new randdelay = RandomEx(978, 1133);
					pData[playerid][pRedMoney] += Rand;
					Robdelay = gettime() + randdelay;
					Server_MinMoney(Rand);
					Server_Save();
...
function Craftdrill(playerid)
{
	if(IsPlayerInRangeOfPoint(playerid, 2.1, -1116.5280, -1636.9052, 76.3671))
	{
		if(pData[playerid][pCraftTime] > 0)
		{
			ApplyAnimation(playerid, "BOMBER", "BOM_Plant", 4.1, 0, 0, 0, 0, 0, 1);
			pData[playerid][pCraftTime]--;
			new mes[128];
			format(mes, sizeof(mes), "Crafting.... ~b~%d", pData[playerid][pCraftTime]);
			InfoTD_MSG(playerid, 1000, mes);
			if(playerid == INVALID_PLAYER_ID || !IsPlayerConnected(playerid))
			{
				KillTimer(pData[playerid][pCraft]);
		...
		{
			KillTimer(pData[playerid][pCraft]);
			pData[playerid][pMaterial] -= 5;
			pData[playerid][pComponent] -= 10;
			pData[playerid][pGas] -= 1;
			pData[playerid][pCraftTime] = 120;

			pData[playerid][pDrill] += 1;
			InfoTD_MSG(playerid, 2000, "You get ~g~1 ~w~Drill");
		}
	}
	else
	{
		KillTimer(pData[playerid][pCraft]);
		pData[playerid][pCraftTime] = 120;
		Error(playerid, "Craft dibatalkan anda menjauh dari lokasi BM!");
	}
	return 1;
}
