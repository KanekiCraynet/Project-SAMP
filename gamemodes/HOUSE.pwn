...
House_Refresh(houseid)
{
    if(houseid != -1)
    {
        ...
        if(strcmp(hData[houseid][hOwner], "-"))
		{
			...
			hData[houseid][hPickup] = CreateDynamicPickup(1272, 23, hData[houseid][hExtposX], hData[houseid][hExtposY], hData[houseid][hExtposZ]+0.2, 0, 0, _, 10.0);
			hData[houseid][hMap] = CreateDynamicMapIcon(hData[houseid][hExtposX], hData[houseid][hExtposY], hData[houseid][hExtposZ], 32, -1, -1, -1, -1, 70.0);
        }
        else
        {
			...
			hData[houseid][hMap] = CreateDynamicMapIcon(hData[houseid][hExtposX], hData[houseid][hExtposY], hData[houseid][hExtposZ], 31, -1, -1, -1, -1, 70.0);
        }
