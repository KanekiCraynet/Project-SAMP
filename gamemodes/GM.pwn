...
#include "FACTIONS.pwn"
#include "INGAMEMAP.pwn"
#include "ACTOR.pwn"
#include "DEBIT.pwn"
#include "ANIMS.pwn"
...
public OnGameModeInit()
{
	...
	mysql_tquery(g_SQL, "SELECT * FROM `faction`", "LoadFactions");
	...
	mysql_tquery(g_SQL, "SELECT * FROM `debit`", "LoadDebit");
	...
	//SetupPlayerTable();
	/*new rent_actor[2];
	rent_actor[0] = CreateActor(27,1682.7796,-2264.0961,13.5116,10.6642);
	SetActorVirtualWorld(rent_actor[0], 0);
	rent_actor[1] = CreateActor(27,1811.7331,-1876.9458,13.5781,274.9719);
	SetActorVirtualWorld(rent_actor[1], 0);*/

	//BM
	BlackMarket = CreateActor(28,-1116.5280,-1636.9052,76.3671,272.9379);
	SetActorVirtualWorld(BlackMarket, 0);
	ApplyActorAnimation(BlackMarket, "COP_AMBIENT", "COPLOOK_LOOP", 4.1, 1, 0, 0, 0, 0);
...
SetPlayerSpawn(playerid)
{
	if(IsPlayerConnected(playerid))
	{
		...
		else
		{
			...
			PlayerTextDrawShow(playerid, tdlok[playerid]);
...
public OnPlayerClickMap(playerid, Float:fX, Float:fY, Float:fZ)
{
	...
	{
		SetPVarFloat(playerid, "pointx", fX);
		SetPVarFloat(playerid, "pointy", fY);
		SetPVarFloat(playerid, "pointz", fZ);
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
    ...
    {	
		...
		for(new i; i < 7; i++)
		{
			PlayerTextDrawHide(playerid, newsveh[playerid][i]);
		}
	...
	else if(newstate == PLAYER_STATE_DRIVER)
    {
		...
		else if(pData[playerid][pHBEMode] == 2)
		{
			...
			for(new i; i < 7; i++)
			{
				PlayerTextDrawShow(playerid, newsveh[playerid][i]);
			}
		}
		else
		{
...
{
	new keypress, ud, lr, ping[80];
	GetPlayerKeys(playerid, keypress, ud, lr);
	if(keypress == KEY_WALK)
	{
		format(ping, sizeof(ping), "Your ping: ~b~%dms", GetPlayerPing(playerid));
		InfoTD_MSG(playerid, 1000, ping);
	}
	//SAPD Tazer/Taser
	...
	if(IsValidVehicle(vehicleid))
	{
		...
		if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		{
			...
			else if(pData[playerid][pHBEMode] == 2)
			{
				...
				PlayerTextDrawSetString(playerid, newsveh[playerid][6], tstr);

				format(tstr, sizeof(tstr), "%.1f%", fDamage);
				PlayerTextDrawSetString(playerid, newsveh[playerid][5], tstr);

				/*format(tstr, sizeof(tstr), "%s", GetVehicleName(vehicleid));
				PlayerTextDrawSetString(playerid, DPvehname[playerid], tstr);*/

				format(tstr, sizeof(tstr), "%.1f Mph", GetVehicleSpeed(vehicleid));
				PlayerTextDrawSetString(playerid, newsveh[playerid][1], tstr);
...
ptask PlayerUpdate[999](playerid)
{
    ...
    {
    	InfoTD_MSG(playerid, 1000, "Type ~p~/rent ~w~to rent bike");
    }
	...
	PlayerTextDrawSetString(playerid, tdlok[playerid], lok);
	...
	if(pData[playerid][pHBEMode] == 1 && pData[playerid][IsLoggedIn] == true)
	{
		...
		new lapar[128], haus[128], kencing[123];
		format(lapar, sizeof(lapar), "%d%", pData[playerid][pHunger]);
		format(haus, sizeof(haus), "%d%", pData[playerid][pEnergy]);
		format(kencing, sizeof(kencing), "%d%", pData[playerid][pBladder]);
		PlayerTextDrawSetString(playerid, PlayerTD[playerid][4], lapar);
		PlayerTextDrawSetString(playerid, PlayerTD[playerid][5], haus);
		PlayerTextDrawSetString(playerid, PlayerTD[playerid][6], kencing);
	}
	else if(pData[playerid][pHBEMode] == 2 && pData[playerid][IsLoggedIn] == true)
	{
		...
		format(haus, sizeof(haus), "%d%", pData[playerid][pEnergy]);
		format(kencing, sizeof(kencing), "%d%", pData[playerid][pBladder]);
		PlayerTextDrawSetString(playerid, newtdhbe[playerid][4], lapar);
		PlayerTextDrawSetString(playerid, newtdhbe[playerid][5], haus);
		PlayerTextDrawSetString(playerid, newtdhbe[playerid][6], kencing);
	}
	else
	{
	...
	if(pData[playerid][pHospital] == 1)
    {
        ...
        if(pData[playerid][pHospitalTime] >= 15)
        {
			...
			ClearAnimations(playerid);
			pData[playerid][pSpawned] = 1;
			PlayerTextDrawHide(playerid, tdblur_0[playerid]);
			SetPVarInt(playerid, "GiveUptime", -1);
		}
    }
	if(pData[playerid][pInjured] == 1 && pData[playerid][pHospital] != 1)
    {
		...
		InfoTD_MSG(playerid, 1000, mstr);
		PlayerTextDrawShow(playerid, tdblur_0[playerid]);
        ...
        ApplyAnimation(playerid, "CRACK", "null", 4.0, 0, 0, 0, 0, 0, 1);
        ApplyAnimation(playerid, "CRACK", "crckdeth4", 4.0, 0, 0, 0, 0, 0, 1);
        SetPlayerHealthEx(playerid, 99999);
    }
